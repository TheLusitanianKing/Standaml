...
let columns_to_display_in_classic_view =
  ...
  ; ( "GD"
    , fun (x : Model.Standing_line.t) ->
        Model.Standing_line.goals_difference x |> Int.to_string )
  ; ( "Points"
    , fun (x : Model.Standing_line.t) ->
        Model.Standing_line.points x |> Int.to_string )
  ]

let prepare_columns columns standing_lines =
  List.map
    ~f:(fun (col_name, get_column) ->
      let col_name_size = String.length col_name in
      let lines_with_sizes =
        List.map
          ~f:(fun sl ->
            let x = get_column sl in
            (Batteries.UTF8.length x, x))
          standing_lines in
      let max_size =
        List.fold lines_with_sizes ~init:col_name_size ~f:(fun x (y, _) ->
            max x y) in
...
let display_standing ~(standing : Model.Standing.t) ~standing_format ~limit =
  ...
  in
  let opt_trimming =
    Option.bind limit ~f:(fun l ->
        Option.return @@ List.take sorted_standing_lines l) in
